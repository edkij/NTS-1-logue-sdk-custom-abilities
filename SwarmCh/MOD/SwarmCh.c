...
/*
 * SwarmCh v0.1
 * Multi-rate stereo "swarm" chorus for the Korg NTS-1 digital mkI MOD slot.
 *
...
void MODFX_PROCESS(const float *main_xn, float *main_yn,
  ...
  for (uint32_t i = 0; i < frames; ++i) {
    ...
    const float in_l = main_xn[s];
    const float in_r = main_xn[s + 1u];
    ...
    main_yn[s]      = in_l * dry_gain + wet_l * wet_gain;
    main_yn[s + 1u] = in_r * dry_gain + wet_r * wet_gain;
